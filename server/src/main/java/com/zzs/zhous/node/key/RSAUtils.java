...
public class RSAUtils {
    ...
    public KeyMap<Key> getKeyMap(){return keyMap;}
		...
		if(keydes==null){
			synchronized(keyMap){
				if(keyMap.get(encryptMapKey)==null){
					...
					if(logger.isInfoEnabled())logger.info(" add decrypt KeyDES, userId={}, appId={}, keyString={} keyId={}",userId,appId,keyString,time);

					keydes=new KeyDES(userId,appId,keyString,session,Cipher.ENCRYPT_MODE,time);
					keyMap.add(encryptMapKey,keydes);
					if(logger.isInfoEnabled())logger.info(" add encrypt KeyDES, userId={}, appId={}, keyString={}, keyId={}",userId,appId,keyString,time);
			...
			if(!time.equals(curKey)) { //keydes.isTimeToUpdate()){ //maybe other thread has been update new aesKeyString
				synchronized(keyMap){
					...
					if(!time.equals(curKey)){ //check again
						...
						if(logger.isInfoEnabled())logger.info(" update decrypt addDES,keyString={} keyId={}",keyString,time);
	
						keydes.add(keyString,time);
						if(logger.isInfoEnabled())logger.info(" update encrypt addDES,keyString={} keyId={}",keyString,time);
					}
				}
			}
		}
		...
		if(keyAes==null){
			synchronized(keyMap){
				if(keyMap.get(encryptMapKey)==null){
					...
					if(logger.isInfoEnabled())logger.info(Thread.currentThread()+" add decrypt keyAES,keyString="+keyString+" keyId="+time);

					keyAes=new KeyAES(userId,appId,keyString,session,Cipher.ENCRYPT_MODE,time);
					keyMap.add(encryptMapKey,keyAes);
					if(logger.isInfoEnabled())logger.info(Thread.currentThread()+" add encrypt keyAES,keyString="+keyString+" keyId="+time);
			...
			synchronized(keyMap){
				if(keyAes.isTimeToUpdate()){ //check again
					...
					if(logger.isInfoEnabled())logger.info(Thread.currentThread()+" update decrypt keyAES,keyString="+keyString+" keyId="+time);

					keyAes.add(keyString,time);
					if(logger.isInfoEnabled())logger.info(Thread.currentThread()+" update encrypt keyAES,keyString="+keyString+" keyId="+time);
				}
			}
		}
	}
...
//	}
	...
	/**
	 * @param rsaRefreshInteral the rsaRefreshInteral to set
	 */
	...
	/**
	 * @param alogrithm the alogrithm to set
	 */
	...
	/**
	 * @param sysAlias the sysAlias to set
	 */
	...
	/**
	 * @param sysAliasPwd the sysAliasPwd to set
	 */
	...
	private Cipher initCipher(String mapkey,String key) {
		...
		try{	
			...
			////TODO fis能返回?
			try{if(fis!=null)fis.close();}catch(Exception e){logger.error("fis.close()error:",e);};
		}
		return cipher;
	}
...
//					}
					...
					if(logger.isDebugEnabled())logger.debug(" first add\nkeyCipher="+keyCipher+"\nwrapper="+wrapper+"\nwrapperBuf="+buf);//+"\n aesKeyLog="+aesKeyLog);
					return wrapper;
				}
			}
		}
...
//			}
			...
			keyCipher.addNewCipher(cipher1,key,null);//aesKeyLog);
			if(logger.isDebugEnabled())logger.debug(" 3rd add\nkeyCipher="+keyCipher+"\nwrapper="+wrapper+"\nwrapperBuf="+buf);//+"\n aesKeyLog="+aesKeyLog);
		...
		try {
			...
			while(t<times){
				...
				if(wrapper==null){
					...
					if(logger.isDebugEnabled())logger.debug(" cannot find any idle cipher for {} keyId={} times={}\n{}",mapKey,keyId,t,wrapBufferMap.get(mapKey).toString());
		...
		try {
			...
			while(t<times){
				...
				if(wrapper==null){
					...
					if(logger.isDebugEnabled())logger.debug(" cannot find any idle cipher for "+mapKey+"keyId="+keyId+"times="+t+"\n"+wrapBufferMap.get(mapKey).toString());
					Thread.currentThread().sleep(50);
					continue;
				}
				cipher=wrapper.getT().getCipher(keyId);
	...
	public String encryptByAES(String plainText,String aesMapKey,String keyId,int times){
		...
		try {
			...
			while(t<times){
				...
				if(wrapper==null){
					if(logger.isDebugEnabled())logger.debug(" cannot find any idle cipher for "+aesMapKey+"keyId="+keyId+"times="+t+"\n"+wrapBufferMap.get(aesMapKey).toString());
					Thread.currentThread().sleep(50);
					continue;
				}
				cipher=wrapper.getT().getCipher(keyId);
	...
	public String encryptByPublicKey(String plainText,String publicMapKey,String publicKey,int times){
		...
		try {
			...
			while(t<times){
				...
				if(wrapper==null){
					if(logger.isDebugEnabled())logger.debug(" cannot find any idle cipher for "+publicMapKey+" time="+t+"\n"+wrapBufferMap.get(publicMapKey).toString());
...
//        cipher.init(Cipher.DECRYPT_MODE, privateKey); 
		...
		try {
			...
			while(t<times){
				...
				if(wrapper==null){
					if(logger.isDebugEnabled())logger.debug(t+" cannot find any idle cipher for "+privateMapKey+"\n"+wrapBufferMap.get(privateMapKey).toString());
	//				privateKey=corePrivateKey;
	//				wrapper=getCipherWrapper(privateKey,keyId);
